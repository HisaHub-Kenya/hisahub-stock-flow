...
// Config
var TOAST_LIMIT = 3; // allow multiple toasts at once
var TOAST_REMOVE_DELAY = 5000; // 5 seconds auto-remove
var actionTypes = {
    ADD_TOAST: "ADD_TOAST",
    ...
    switch (action.type) {
        ...
        case actionTypes.DISMISS_TOAST: {
            ...
            if (toastId_1) {
                ...
                state.toasts.forEach(function (toast) { return addToRemoveQueue(toast.id); });
            }
            return __assign(__assign({}, state), { toasts: state.toasts.map(function (t) {
                    return toastId_1 === undefined || t.id === toastId_1
...
function dispatch(action) {
    ...
    listeners.forEach(function (listener) { return listener(memoryState); });
}
function toast(_a) {
    var props = __rest(_a, []);
    var id = genId();
    ...
    var update = function (newProps) {
        return dispatch({
            type: actionTypes.UPDATE_TOAST,
            toast: __assign(__assign({}, newProps), { id: id }),
        });
    };
    dispatch({
...
function useToast() {
    ...
    return __assign(__assign({}, state), { toast: toast, dismiss: function (toastId) {
